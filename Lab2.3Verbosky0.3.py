...
print(firstName[0].lower() + firstName[1:].upper() +\
", " + lastName[0].lower() + lastName[1:].upper()) 
print()
...
phraseEncrypted = phraseUppered.replace("A","1").replace("E","2").replace("I","3").replace("O","4").replace("U","5")
...
print("Task #3")
length = len(phrase)
phraseFirst = phrase[0:length//3]
phraseSecond = phrase[length//3:length//3*2]
phraseThird = phrase[length//3*2:length-1]
...
digit2 = int(fiveDNumber[len(fiveDNumber)//5:len(fiveDNumber)//5*2])
digit3 = int(fiveDNumber[len(fiveDNumber)//5*2:len(fiveDNumber)//5*3])
digit4 = int(fiveDNumber[len(fiveDNumber)//5*3:len(fiveDNumber)//5*4])
digit5 = int(fiveDNumber[len(fiveDNumber)//5*4:len(fiveDNumber)])
